...
def run_kf_case(case_name, enable_outage, dt, accel, gyro, imu_time,
    ...
    for k in range(len(imu_time)):
        ...
        if next_gnss_idx < len(rtk_time) and t >= rtk_time[next_gnss_idx]:
            #update like normal if not in outage
            if not in_outage:
                #update with rtk solutions
                y, S = kf.update(rtk_pos[next_gnss_idx], rtk_vel[next_gnss_idx])
...
def main():
    ...
    #added a suubfodler for rtk and rtk ekf plots to declutter folder
    plot_dir = Path("Plots") / f"Run_{run_id}" / "RTK_KF"
    ...
    rtk["datetime"] = pd.to_datetime(rtk["datetime"]) #rtk datetime moved up for start_time comp
    ...
    # Crop rtk to same start time as IMU/GT
    rtk = rtk[rtk["datetime"] > start_time].reset_index(drop=True)

    # Drop first row if velocity interpolation produced NaN
    rtk = rtk.dropna(subset=[
        "X-ECEF", "Y-ECEF", "Z-ECEF",
        "VX-ECEF", "VY-ECEF", "VZ-ECEF"
    ]).reset_index(drop=True)

    rtk_pos = rtk[["X-ECEF", "Y-ECEF", "Z-ECEF"]].to_numpy()
    rtk_vel = rtk[["VX-ECEF", "VY-ECEF", "VZ-ECEF"]].to_numpy()

    rtk_n, rtk_e, rtk_d = pm.ecef2ned(
    ...
    print("RTK-only RMSE:", np.sqrt(np.mean(np.sum(rtk_err ** 2, axis=1))))
    print("Mean RTK N/E/D error:", np.mean(rtk_err, axis=0))

    rtk_time = (rtk["datetime"] - start_time).dt.total_seconds().to_numpy()

    #getting R (variance for rtk) for std in rtk_solution csv
    #using the avaerage std
    pos_var = rtk[["std_X", "std_Y", "std_Z"]].pow(2).mean().to_numpy()
    vel_var = rtk[["std_VX", "std_VY", "std_VZ"]].pow(2).mean().to_numpy()

    R = np.zeros((6, 6))
    R[0:3, 0:3] = np.diag(pos_var) # position noise (variance)
    R[3:6, 3:6] = np.diag(vel_var) # velocity noise (variance)

    #allign imu time with same ref as rtk
    imu_time = (imu["datetime"] - start_time).dt.total_seconds().to_numpy()

    #run kf without outage
    case_name_no_outage = f"Run_{run_id}_RTK_NoOutage"
    ...
    # run kf with outage
    case_name_outage = f"Run_{run_id}_RTK_Outage"
