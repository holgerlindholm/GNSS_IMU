...
def main():
    ...
    spp_file = "data/run2_spp_solution.csv"
    ...
    # Limit to first ~20 seconds after start_time
    imu = imu.iloc[:2000].reset_index(drop=True)
    gt = gt.iloc[:2000].reset_index(drop=True)
    ...
    # QUICK TEST: remove gravity (assuming Z-up / NEU frame)
    #accel[:, 2] -= 9.81
    ...
    for k in range(len(imu)):
        ...
        if use_gnss_updates and next_gnss_idx < len(spp_time) and t >= spp_time[next_gnss_idx]:
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('Plots/INS_Mechanization_Check_Run_2.png', dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.title("KF ECEF delta_X and delta_Y")
    plt.grid(True)
    plt.legend()
    #plt.savefig('Plots/ECEF_deltaXY_SPP_Run_2.png', dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.grid(True)
    plt.legend()
    #plt.savefig('Plots/NIS_SPP_Run_2.png', dpi=300, bbox_inches="tight")
    plt.show()
    ...
    plt.grid(True)
    plt.legend()
    #plt.savefig('Plots/Position_RMSE_SPP_Run_2.png', dpi=300, bbox_inches="tight")
    plt.show()
